...
class Regular extends Component {
  render() {
    const { regPictures } = this.props;
    const regList = regPictures.length ? (
      regPictures.map((picture) => {
        return (
          <div key={picture.id} className="pictureBox">
            ...
            <div className="voteContainer">
              ...
              >
                <i className="fa fa-thumbs-down" style={{ fontSize: 20 }}></i>{" "}
                {picture.downvotes}
              </button>
            </div>
          </div>
      ...
      <main>
        <h3>NO REGULAR PICTURES</h3>
      </main>
    );
    return <main>{regList}</main>;
  }
}

const mapStateToProps = (state) => {
  return {
    regPictures: state.pictures.filter(
      (regFilterdPicture) =>
        regFilterdPicture.upvotes - regFilterdPicture.downvotes < 5
    ),
  };
};
